 ...
 * @Date 2021/9/12 21:24
 */
public class LemonAdd extends BaseCase {
    ...
    public void testAdd(ExcelData excelData) throws JsonProcessingException, FileNotFoundException {
        ...
        if (sqlCheck != null){
            ...
            if (excelData.getCase_id()==1){
                GlobalEnvironment.envVariable.put("loan_id_investor",res.path("data.id"));
            }else if (excelData.getCase_id()==2){
                GlobalEnvironment.envVariable.put("loan_id_borrower",res.path("data.id"));
            }
