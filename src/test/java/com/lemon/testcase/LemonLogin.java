 ...
 * @Date 2021/9/7 17:02
 */
public class LemonLogin extends BaseCase {
    ...
    public void testLogin(ExcelData excelData) throws JsonProcessingException, FileNotFoundException {
        ...
        if (excelData.getCase_id()==1){
                    GlobalEnvironment.envVariable.put("investor_token",res.path("data.token_info.token"));
                    GlobalEnvironment.envVariable.put("member_id_investor",res.path("data.id"));
        }else if (excelData.getCase_id()==2){
                    GlobalEnvironment.envVariable.put("borrower_token",res.path("data.token_info.token"));
                    GlobalEnvironment.envVariable.put("member_id_borrower",res.path("data.id"));
        }else if (excelData.getCase_id()==3){
                    GlobalEnvironment.envVariable.put("admin_token",res.path("data.token_info.token"));
                    GlobalEnvironment.envVariable.put("member_id_admin",res.path("data.id"));
        }
